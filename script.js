// ============================================================
//  BARIK DEVELOPER — AM Generator
// ============================================================

const $ = (id) => document.getElementById(id);
const API_BASE = '';

const PRICE_MANUAL = 1000;
const PRICE_AUTO   = 500;
const MIN_QTY = 1, MAX_QTY = 20;
const VERIF_TIMEOUT = 5 * 60 * 1000;

const STORAGE_KEY = 'barikdev_pending_tx';
...
(function initWelcome() {
  ...
  if (titleEl) titleEl.textContent = `Selamat ${greeting}, Sobat Barik Developer`;
...
function applyTheme(name) {
  ...
  localStorage.setItem('barikdev_theme', name);
}
window.cycleTheme = function() {
  const cur = localStorage.getItem('barikdev_theme') || 'yellow';
  const idx = THEMES.indexOf(cur);
  const next = THEMES[(idx + 1) % THEMES.length];
  applyTheme(next);
  log(`[THEME] Tema: ${next}`);
};
applyTheme(localStorage.getItem('barikdev_theme') || 'yellow');
...
const GUIDE_DATA = {
  manual: {
    ...
    sub: 'Pakai email sendiri · Rp 1.000',
    body: `
      ...
      <div class="g-item">
        ...
        <div>
          ...
          <span>Nanti muncul konfirmasi, cek lagi totalnya Rp 1.000. Kalau udah bener, lanjut bayar.</span>
        </div>
      </div>
  ...
  auto: {
    title: 'Beli Alight Motion',
    sub: 'Email digenerate sistem · Rp 500/akun',
    body: `
      ...
      <div class="g-item">
        ...
        <div>
          ...
          <span>Bisa beli 1 sampai 20 akun sekaligus. Harga Rp 500 per akun.</span>
        </div>
      </div>
...
const SEWA_LIST_KEY = 'barikdev_sewa_list';
...
if (confirmPayBtn) confirmPayBtn.addEventListener('click', async () => {
  ...
  try {
    ...
    if (mode === 'manual') {
      ...
      sendBtn.innerHTML = '<i class="fas fa-shopping-cart"></i> Beli Sekarang Rp 1.000';
    } else {
...
function showPayModal(tx, modeOverride) {
  ...
  if (mode === 'donasi') productText = 'Donasi Barik Developer';
...
window.downloadPaidTxt = function() {
  const rows = paidState.results.filter(r => r.ok);
  if (!rows.length) { showMessage(msgBoxPaid, 'error', 'Belum ada akun berhasil.'); return; }
  let c = `BARIK DEVELOPER — Akun Premium (Auto)\n`;
  c += `Tanggal : ${new Date().toLocaleString('id-ID')}\n`;
  c += `Total   : ${rows.length} akun × Rp 500\n`;
  c += `${'='.repeat(50)}\n\n`;
  ...
  a.href = url; a.download = `barik-developer-auto-${Date.now()}.txt`; a.click();
