// ============================================================
//  ALIGHT MOTION SCRAPER — amp-logic.js
// ============================================================
...
class AlightMotionAuth {
  constructor() {
    ...
    this.SKU_TYPE   = 'subs';
    this.ORDER_ID   = 'BY-BARIKZ-GANTENG';
  ...
  async sendMagicLink(email) {
    try {
      ...
      await axios.post(
        `${this.IDENTITY_BASE}/createAuthUri?key=${this.API_KEY}`,
        { identifier: cleanEmail, continueUri: 'http://localhost' },
        { headers: this.ANDROID_HEADERS, timeout: 15000 }
      );

      await axios.post(
        `${this.IDENTITY_BASE}/getOobConfirmationCode?key=${this.API_KEY}`,
        {
      ...
      return { success: false, error: this._cleanErr(error) };
    }
  }
  ...
  extractOobCode(rawUrl) {
    ...
    try {
      ...
      const m =
        url.match(/[?&]oobCode=([a-zA-Z0-9_-]+)/i) ||
        url.match(/oobCode=([a-zA-Z0-9_-]+)/i);
  ...
  async verifyAndFetchProfile(email, rawLink) {
    try {
      ...
      if (!oobCode) {
        return { success: false, error: 'oobCode tidak ditemukan di link.' };
      }

      const cleanEmail = email.trim().toLowerCase();

      const signinRes = await axios.post(
        `${this.IDENTITY_BASE}/emailLinkSignin?key=${this.API_KEY}`,
      ...
      try {
        const accRes = await axios.post(
          `${this.IDENTITY_BASE}/getAccountInfo?key=${this.API_KEY}`,
          { idToken },
          { headers: this.ANDROID_HEADERS, timeout: 15000 }
        );
