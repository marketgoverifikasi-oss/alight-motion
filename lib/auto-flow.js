...
async function pollInboxOnce(email, knownIds = []) {
  ...
  if (!newMsgs.length) return { hasNew: false, messages: [], knownIds: messages.map(m => m.mail_id) };
  const details = await Promise.all(newMsgs.map(async (msg) => {
    try { return await tempMail.getMail(email, msg.mail_id); } catch { return msg; }
  }));
  return { hasNew: true, messages: details, knownIds: messages.map(m => m.mail_id) };
}
  ...
  for (const mail of mails) {
    ...
    for (const content of candidates) {
      ...
      for (const u of urls) {
        ...
        if (/oobCode=/.test(cleaned) || /alightcreative\.com/.test(cleaned) ||
            /identitytoolkit/.test(cleaned) || /firebaseapp\.com/.test(cleaned)) {
...
module.exports = { createTempEmail, pollInboxOnce, findMagicLink, clampBatch, MAX_BATCH };